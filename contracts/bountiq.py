...
def extract_json_object(text: str) -> dict:
    ...
    if not isinstance(text, str):
        raise gl.UserError("Model returned a non-text response")
    start = text.find("{")
    end = text.rfind("}")
    if start == -1 or end == -1 or end < start:
        raise gl.UserError("Model response contained no JSON object")
    try:
        return json.loads(text[start : end + 1])
    except Exception as exc:
        raise gl.UserError(f"Model response was not valid JSON: {exc}")
...
def normalize_verdict(raw) -> dict:
    ...
    if not isinstance(raw, dict):
        raise gl.UserError("Model verdict must be a JSON object")

    verdict = str(raw.get("verdict", "")).strip().lower()
    if verdict not in VERDICTS:
        raise gl.UserError(f"Model returned an unsupported verdict: {verdict!r}")

    try:
        score = int(round(float(str(raw.get("score", 0)).strip())))
    except (TypeError, ValueError):
        raise gl.UserError("Model verdict contained a non-numeric score")
    ...
    if not results:
        raise gl.UserError("Model verdict did not assess any acceptance criteria")
...
class Bountiq(gl.Contract):
    ...
    def _bounty(self, bounty_id: str) -> Bounty:
        if bounty_id not in self.bounties:
            raise gl.UserError(f"Unknown bounty: {bounty_id}")
        return self.bounties[bounty_id]

    def _submission(self, submission_id: str) -> Submission:
        if submission_id not in self.submissions:
            raise gl.UserError(f"Unknown submission: {submission_id}")
...
Include one entry in criteriaResults for each acceptance criterion, in the same order."""
        ...
        if not isinstance(verdict, dict):
            raise gl.UserError("Consensus did not produce a usable verdict")
        return verdict
    ...
    def _require_requester(self, submission: Submission) -> Bounty:
        ...
        if bounty.requester != gl.message.sender_address:
            raise gl.UserError("Only the bounty creator can manage its submissions")
        return bounty
    ...
    def create_bounty(
        ...
        if not title or len(title) > MAX_TITLE_CHARS:
            raise gl.UserError(f"Title must be between 1 and {MAX_TITLE_CHARS} characters")
        if not brief or len(brief) > MAX_BRIEF_CHARS:
            raise gl.UserError(f"Brief must be between 1 and {MAX_BRIEF_CHARS} characters")
        if not deadline:
            raise gl.UserError("Deadline is required")
        if len(brand) > MAX_BRAND_CHARS:
            raise gl.UserError(f"Brand name must be at most {MAX_BRAND_CHARS} characters")
        if len(brand_url) > MAX_URL_CHARS:
            raise gl.UserError(f"Brand link must be at most {MAX_URL_CHARS} characters")

        try:
            reward_wei = int(str(reward).strip())
        except (TypeError, ValueError):
            raise gl.UserError("Reward must be a whole number of wei")
        if reward_wei <= 0:
            raise gl.UserError("Reward must be greater than zero")

        try:
            wanted = int(slots)
        except (TypeError, ValueError):
            raise gl.UserError("Submissions wanted must be a whole number")
        if wanted < 1 or wanted > MAX_SLOTS:
            raise gl.UserError(f"Submissions wanted must be between 1 and {MAX_SLOTS}")

        try:
            pool = int(winners)
        except (TypeError, ValueError):
            raise gl.UserError("Winners must be a whole number")
        if pool < 0 or pool > MAX_SLOTS:
            raise gl.UserError(f"Winners must be between 0 (unlimited pool) and {MAX_SLOTS}")

        try:
            parsed = json.loads(criteria)
        except Exception:
            raise gl.UserError("Acceptance criteria must be a JSON array of strings")
        ...
        for item in parsed:
            ...
            if len(text) > MAX_CRITERION_CHARS:
                raise gl.UserError(f"Each criterion must be at most {MAX_CRITERION_CHARS} characters")
            cleaned.append(text)
        if not cleaned:
            raise gl.UserError("At least one acceptance criterion is required")
        if len(cleaned) > MAX_CRITERIA:
            raise gl.UserError(f"At most {MAX_CRITERIA} acceptance criteria are supported")
        ...
        if sent > required:
            raise gl.UserError(f"Send exactly {required} wei to fully fund this bounty")
    ...
    def fund_bounty(self, bounty_id: str) -> None:
        ...
        sent = int(gl.message.value)
        if sent == 0:
            raise gl.UserError("Send GEN to fund this bounty")

        winners = int(bounty.winners)
        required = int(bounty.reward) * (winners if winners > 0 else int(bounty.slots))
        if int(bounty.escrow) + sent > required:
            raise gl.UserError(f"This bounty only needs {required} wei in total")
    ...
    def submit_work(self, bounty_id: str, content: str) -> str:
        ...
        if str(bounty.status) != "open":
            raise gl.UserError("This bounty is no longer accepting submissions")

        content = content.strip()
        if not content:
            raise gl.UserError("A deliverable is required")
        if len(content) > MAX_SUBMISSION_CHARS:
            raise gl.UserError(f"Deliverable must be at most {MAX_SUBMISSION_CHARS} characters")

        existing = json.loads(str(bounty.submission_ids))
        if len(existing) >= int(bounty.slots):
            raise gl.UserError("This bounty has all the submissions it asked for")
    ...
    def resubmit_work(self, submission_id: str, content: str) -> None:
        ...
        if submission.contributor != gl.message.sender_address:
            raise gl.UserError("Only the contributor can resubmit this work")
        if str(submission.payment) in ("approved", "paid"):
            raise gl.UserError("This submission has already been rewarded")
        if str(submission.verdict) == "approved":
            raise gl.UserError("This submission already passed verification")
    ...
    def review_submission(self, submission_id: str, decision: str) -> None:
        ...
        if decision not in DECISIONS:
            raise gl.UserError("Decision must be 'accepted' or 'declined'")
    ...
    def approve_payment(self, submission_id: str) -> None:
        ...
        if str(submission.review) != "accepted":
            raise gl.UserError("Accept the submission before approving its payment")
        ...
        if winners > 0 and rewarded >= winners and str(submission.payment) not in ("approved", "paid"):
            raise gl.UserError(f"This bounty rewards {winners} winner(s) and the pool is full")
        if already < int(bounty.reward):
            raise gl.UserError("This bounty does not have enough escrow left for another reward")
    ...
    def mark_paid(self, submission_id: str) -> None:
        ...
        if str(submission.payment) != "approved":
            raise gl.UserError("Approve the payment before marking it paid")

        reward = int(bounty.reward)
        escrow = int(bounty.escrow)
        if escrow < reward:
            raise gl.UserError("This bounty's escrow is exhausted")
